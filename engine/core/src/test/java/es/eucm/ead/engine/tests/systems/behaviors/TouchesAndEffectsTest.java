...
public class TouchesAndEffectsTest extends BehaviorTest implements
	...
	public void testTargets() {
		...
		testTargetEffectExecution("all", owner, actorEntity1, actorEntity2,
				actorEntity3, owner);
		testTargetEffectExecution("this", owner, owner);

		// Test valid "each entity"
		testTargetEffectExecution("each entity {(hastag $entity stag1)}",
				owner, actorEntity1);
		testTargetEffectExecution(
				"each entity {(and (not (hastag $entity stag2))  (hastag $entity stag3))}",
				owner, actorEntity3);
		// In the next try, since $this = actorEntity3, (not (hastag $this
		// stag2)) is equivalent to btrue. It is just to test that effectsSystem
		// is able to resolve $this and $entity at the same time.
		testTargetEffectExecution(
				"each entity {(and (not (hastag $this stag2))  (hastag $entity stag3))}",
				actorEntity3, actorEntity1, actorEntity2, actorEntity3);

		// Test not valid "each entity"
		testTargetEffectExecution("each entity {(hastag $entity tag1)}", owner); // Bad
																					// tag
		testTargetEffectExecution("each entity (hastag $entity stag1)", owner); // No
																				// {}
		testTargetEffectExecution("eachentity {(hastag $entity stag1)}", owner); // eachentity
																					// altogether
	}

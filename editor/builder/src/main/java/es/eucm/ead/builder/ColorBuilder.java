...
public class ColorBuilder {
    ...
    public Color hex(String hex) {
        if (hex.length()!=3 && hex.length()!=6 && hex.length()!=8){
            throw new IllegalArgumentException("The hex value is not correct. Should have 3, 6 or 8 hex characters (not including the starting # character): "+hex);
        }
        hex = hex.toLowerCase();
        for (int i=0; i<hex.length(); i++) {
            char c = hex.charAt(i);
            if ((c<'0' || c>'9') && (c<'a' || c>'f')) {
                throw new IllegalArgumentException("The hex value is not correct. Only Hex characters 0-9 A-F can be used");
            }
        }
    ...
    public com.badlogic.gdx.graphics.Color toGdx(Color color) {
        return new com.badlogic.gdx.graphics.Color(color.getR(), color.getG(), color.getB(), color.getA());
    }

    private void checkRange(String attribute, int value) {
        if (value<0 || value>255){
            throw new IllegalArgumentException("Color could not be created. Attribute "+attribute+" has an invalid value. Must be between 0 and 255");
        }
    }

    private void checkRange(String attribute, float value) {
        if (value<0F || value>1.0F){
            throw new IllegalArgumentException("Color could not be created. Attribute "+attribute+" has an invalid value. Must be between 0.0 and 1.0");
        }
    }
}
